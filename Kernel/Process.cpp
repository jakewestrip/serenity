...
void signal_trampoline_dummy()
{
    ...
    constexpr static auto offset_to_first_register_slot = sizeof(__ucontext) + sizeof(siginfo) + 6 * sizeof(FlatPtr);
    asm(
        ...
        "pop ecx\n" // save handler
        // save ebp into the first register slot
        "mov [esp+%P1], ebp\n"
        "mov ebp, esp\n"
    ...
    asm(
        ...
        ".globl asm_signal_trampoline\n"
        "asm_signal_trampoline:\n"
        // stack state: 0, 0, ucontext, signal_info (alignment = 16), ucontext*, siginfo*, signal, handler

        // Pop the handler into rcx
        "pop rcx\n" // save handler
        // save rbp into the first register slot
        "mov [rsp+%P1], rbp\n"
        ...
        // call the signal handler
        "call rcx\n"
        // Current stack state is just saved_rbp, saved_rax, ucontext, signal_info.
        // syscall SC_sigreturn
        "mov rax, %P0\n"
        "int 0x82\n"
        ".globl asm_signal_trampoline_end\n"
